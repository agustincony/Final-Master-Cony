...
# Semáforo ASIMÉTRICO (Opción A) con cortes fisiológicos del ACWR:
#   zona segura ~0.80-1.30 | sobrecarga 1.30-1.50 | riesgo alto >1.50
#   descarga (ratio bajo) = naranja/violeta: se ve, no penaliza.
def color_ratio(r):
    if r is None or np.isnan(r): return "#4a6a80"
    if r >= 1.50: return "#FF0000"   # riesgo alto
    if r >= 1.30: return "#FFD000"   # sobrecarga
    if r >= 0.80: return "#00CC44"   # zona segura
    if r >= 0.60: return "#FF8C00"   # descarga leve (naranja)
    return "#8A2BE2"                  # descarga alta (violeta)

def score_ratio(r):
    # Solo la sobrecarga (ratio alto) suma riesgo. La descarga no penaliza.
...
# El score de alerta se calcula sobre 4 ejes independientes, no sobre las 10
# métricas correlacionadas. Cada eje es la suma cruda de sus métricas base.
EJES = {
    "Volumen":       ["Distancia Total"],
    "Intensidad":    ["AI 18 Km/h"],
    "Neuromuscular": ["Acel 2,5 m/ss #", "Desacel -2,5 m/ss #"],
    "Colisión":      ["Contact Involvement Total Count Avg"],
}
EJES_LABELS = {
    "Volumen":       ("Volumen",       "#7FB3E0"),
    "Intensidad":    ("Intensidad",    "#F2A8C0"),
    "Neuromuscular": ("Neuromuscular", "#A8DDB5"),
    "Colisión":      ("Colisión",      "#A8E0DC"),
}
...
def calcular_ewma(df):
    ...
    Se usa para los gráficos de detalle (mantiene las 10 métricas).
    Días sin sesión = 0 (el descanso descarga la aguda).
    """
    ...
    for jugador, g in agg.groupby("Player Name"):
        ...
        for col in COLS:
            g_diario[col] = g_diario[col].fillna(0)   # OPCIÓN A: descanso = 0
...
def calcular_ewma_ejes(df):
    """
    EWMA aguda (7) / crónica (28) por jugador y EJE.
    Cada eje = suma cruda de sus métricas base por jugador+fecha.
    Días sin sesión = 0 (OPCIÓN A: el descanso descarga la aguda).
    """
    ...
    for jugador, g in agg.groupby("Player Name"):
        ...
        for eje in COLS_EJES:
            g_diario[eje] = g_diario[eje].fillna(0)   # OPCIÓN A: descanso = 0
...
# Pivot: jugador × eje → ratio
pivot = df_fecha.pivot_table(index=["Player Name", "Position Name"], columns="Eje", values="Ratio").reset_index()

# Status por eje (score_ratio: solo la sobrecarga suma)
for eje in COLS_EJES:
    if eje in pivot.columns:
        pivot[f"score_{eje}"] = pivot[eje].apply(score_ratio)
    else:
        pivot[f"score_{eje}"] = 1

# Umbrales: 4 ejes, status 1-3 c/u -> total 4..12.
N_EJES = len(COLS_EJES)
UMBRAL_VERDE = 6   # <=6 óptimo
UMBRAL_AM    = 8   # 7-8 precaución; >8 alerta

def calcular_score_ejes(row):
    scores = [row[f"score_{eje}"] for eje in COLS_EJES]
    total = sum(scores)
    n = len(scores)
    if total <= UMBRAL_VERDE:
        return total, n, "#00CC44", "✅ Óptimo"
    if total <= UMBRAL_AM:
        return total, n, "#FFD000", "⚠️ Precaución"
    return total, n, "#FF0000", "🚨 Alerta"
...
if df_jug_met.empty:
    ...
    if not row_actual.empty:
        score_actual = int(row_actual["Score_Total"].iloc[0])
        color_actual = row_actual["Color_Score"].iloc[0]
        label_actual = row_actual["Label_Score"].iloc[0]
    else:
        score_actual = None
        color_actual = "#4a6a80"
        label_actual = "—"

    # Header del jugador: ratio EWMA por EJE (lo que arma el score)
    metricas_html = ""
    for eje in COLS_EJES:
        ...
        val_str = "—" if (ratio_m is None or np.isnan(ratio_m)) else f"{ratio_m:.2f}"
        metricas_html += (
