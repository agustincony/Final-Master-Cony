...
.tabla-semaforo th.col-jugador { text-align: left; min-width: 80px; max-width: 180px; }
.tabla-semaforo td { padding: 6px 6px; border-bottom: 1px solid #1e3048; text-align: center; color: white; }
.tabla-semaforo td.col-jugador { text-align: left; color: #cce0f0; font-weight: 600; max-width: 180px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
::-webkit-scrollbar-track { background: #0f1a28; }
::-webkit-scrollbar-thumb { background: #00A8CC; border-radius: 4px; }
...
<div class="topbar">
    ...
    <div class="topbar-divider"></div>
    <span class="topbar-page">Análisis de Sesión</span>
</div>
""", unsafe_allow_html=True)
...
def render_filtro_puesto(col_ctx, grupos, prefix, opciones_validas):
    grupos_sel = [g for g in grupos.keys() if st.session_state.get(f"{prefix}grupo_{g}", False)]
    btn_txt = "Todos ▾" if not grupos_sel else (f"{grupos_sel[0]} ▾" if len(grupos_sel) == 1 else f"{len(grupos_sel)} selec. ▾")
    with col_ctx:
        ...
        with st.popover(btn_txt, use_container_width=True):
            ...
            for grupo, puestos in grupos.items():
                ...
                if not puestos_validos:
                    continue
                st.checkbox(grupo.upper(), key=f"{prefix}grupo_{grupo}")
...
grupos_sel_is = [g for g in GRUPOS_PUESTO.keys() if st.session_state.get(f"pue_grupo_{g}", False)]
pue_sel = [p for g in grupos_sel_is for p in GRUPOS_PUESTO[g]]
...
import datetime
df_fec = df_base.copy()
if md_activo:  df_fec = df_fec[df_fec["MD"].isin(md_activo)]
...
if len(jug_sel) == 1 and _agg.empty:
    st.markdown('<div style="background:#FFD00022;border:1px solid #FFD000;border-radius:8px;padding:12px;color:#FFD000;font-size:13px;font-weight:700;">⚠️ Jugó menos de 30 minutos — ver detalle abajo en la tabla</div>', unsafe_allow_html=True)
else:
    cols1 = st.columns(8)
...
for i, col in enumerate(COLS):
    ...
    if pct_num is not None and pct_num > 130:
        ...
        alerta = ''
    pct_str = f"{pct_num:.0f}%" if pct_num is not None and not np.isnan(avg_sesion[col]) else ""
    with cols2[i]:
        st.markdown(f"""
        <div class="tarjeta-comp" style="background:{fondo}; border:2px solid {borde};">
            ...
            <div class="valores">{"" if np.isnan(val) else f"{fmt(val)} / {fmt(bench)}"}</div>
...
NOMBRES_TABLA   = ["Jugador", "Puesto", "Fecha", "Rival", "Equipo", "MD",
                   "Minutos", "Dist Tot", "HSR", "Sprint",
                   "N° Sprints", "Acel", "Decel", "Contactos"]
...
for c in ["Minutos", "Dist Tot", "HSR", "Sprint", "N° Sprints", "Acel", "Decel", "Contactos"]:
...
cols_met_tabla = ["Minutos", "Dist Tot", "HSR", "Sprint", "N° Sprints", "Acel", "Decel", "Contactos"]
cols_met_tabla = [c for c in cols_met_tabla if c in df_tabla.columns]

cabecera = "<tr><th class='col-jugador'>Jugador</th><th>Fecha</th><th>Rival</th><th>Equipo</th><th>MD</th>"
for c in cols_met_tabla:
    cabecera += f"<th>{c}</th>"
cabecera += "</tr>"

filas = ""
for _, row in df_tabla.iterrows():
    fecha_str = pd.Timestamp(row["Fecha"]).strftime("%d/%m/%Y") if pd.notna(row["Fecha"]) else ""
    fila = (
        f"<tr><td class='col-jugador'>{row.get('Jugador','')}</td>"
        f"<td>{fecha_str}</td>"
        f"<td>{row.get('Rival','')}</td>"
        f"<td>{row.get('Equipo','')}</td>"
        f"<td>{row.get('MD','')}</td>"
    )
    for c in cols_met_tabla:
        val = row.get(c, None)
...
st.markdown(
    "<div style='overflow-x:auto;overflow-y:auto;max-height:400px;'><table class='tabla-semaforo'>" + cabecera + filas + "</table></div>",
    unsafe_allow_html=True
)
