...
# Semáforo SIMÉTRICO (Opción B) con cortes fisiológicos del ACWR.
def color_ratio(r):
    if r is None or np.isnan(r): return "#4a6a80"
    if 0.80 <= r <= 1.30: return "#00CC44"
    if 0.60 <= r <= 1.50: return "#FFD000"
    return "#FF0000"

def score_ratio(r):
    if r is None or np.isnan(r): return 1
    if 0.80 <= r <= 1.30: return 1
    if 0.60 <= r <= 1.50: return 2
    return 3
...
def calcular_ewma(df):
    ...
    Días sin sesión = NaN (el descanso NO pesa).
    """
    ...
    for jugador, g in agg.groupby("Player Name"):
        ...
        # OPCIÓN B: sin fillna, los días sin sesión quedan NaN (descanso no pesa)
...
def calcular_ewma_ejes(df):
    ...
    Días sin sesión = NaN (OPCIÓN B: el descanso NO pesa).
    """
