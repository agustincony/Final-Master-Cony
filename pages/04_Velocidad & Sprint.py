...
st.markdown(
    ...
    f'<div class="topbar-divider"></div><span class="topbar-page">Velocidad & Sprint</span></div>',
    unsafe_allow_html=True
)
...
if df_top_base.empty:
    ...
    for _, row in df_top.iterrows():
        hover_texts.append(
            ...
            f"Velocidad: {row['Maximum Velocity']:.2f}".replace(".", ",") + f" km/h ({row['Max Vel (% Max)']:.0f}%)<br>"
...
for _, row in df_top.iterrows():
    hover_texts.append(
        f"<b>{row['Player Name']}</b><br>"
        f"Velocidad: {row['Maximum Velocity']:.2f}".replace(".", ",") + f" km/h ({row['Max Vel (% Max)']:.0f}% max)<br>"
        f"Actividad: {row['Activity Name']}<br>"
        f"Fecha: {row['Fecha']}<br>"
        f"Tipo: {'Partido' if row['Es_Partido'] else 'Entrenamiento'}"
    )
...
st.markdown('<div class="seccion-header">🔍 Búsqueda de velocidad máxima</div>', unsafe_allow_html=True)
st.markdown('<span style="font-size:15px;color:#7a9ab5;">Elegí uno o varios jugadores</span>', unsafe_allow_html=True)

jugadores_bus = [""] + sorted(df_raw["Player Name"].dropna().unique().tolist())
cols_bus = st.columns(5)
jugadores_sel = []

for i, col in enumerate(cols_bus):
    with col:
        jug = st.selectbox("", jugadores_bus, label_visibility="collapsed", key=f"bus_jug_{i}")
        jugadores_sel.append(jug)

cols_tar = st.columns(5)
for i, jug_sel in enumerate(jugadores_sel):
    with cols_tar[i]:
        if jug_sel:
            df_jug_vel = df_raw[df_raw["Player Name"] == jug_sel].sort_values("Maximum Velocity", ascending=False).iloc[0]
            vel_max = df_jug_vel["Maximum Velocity"]
            fecha_max = pd.Timestamp(df_jug_vel["Fecha"]).strftime("%d/%m/%Y")
            actividad_max = df_jug_vel["Activity Name"]
            st.markdown(
                f'<div style="background:#0f1a28;border:1px solid #1e3048;border-radius:12px;padding:16px;">'
                f'<div style="font-size:12px;font-weight:900;color:#ffffff;margin-bottom:8px;">{jug_sel}</div>'
                f'<div style="font-size:26px;font-weight:900;color:#00A8CC;">{vel_max:.2f}'.replace(".", ",") + f' <span style="font-size:12px;">km/h</span></div>'
                f'<div style="font-size:10px;color:#4a6a80;margin-top:6px;">{actividad_max}</div>'
                f'<div style="font-size:10px;color:#4a6a80;">{fecha_max}</div>'
                f'</div>',
                unsafe_allow_html=True
            )
...
METRICAS_SEM = {
    "Velocidad máxima":   ("Max Vel (% Max)",  "valor_directo"),
    "Aceleración máxima":  ("Max Acceleration",  "max_personal"),
    "Deceleración máxima": ("Max Deceleration",  "min_personal"),
}
...
if df_fecha.empty:
    ...
    agg_func = "max" if col_metrica in ["Maximum Velocity", "Max Acceleration", "Max Deceleration", "Max Vel (% Max)"] else "sum"
    df_val_sesion = df_fecha.groupby(["Player Name", "Position Name"]).agg(
        Valor_Sesion=(col_metrica, agg_func),
        Velocidad_kmh=("Maximum Velocity", "max")
    ).reset_index()
    ...
    elif tipo_ref == "min_personal":
        ref = df_hist.groupby("Player Name")[col_metrica].min()
    elif tipo_ref == "valor_directo":
        ref = pd.Series(100, index=df_hist.groupby("Player Name")[col_metrica].max().index)
    else:
    ...
    if tipo_ref == "min_personal":
        df_val_sesion["Pct"] = (df_val_sesion["Valor_Sesion"].abs() / df_val_sesion["Referencia"].abs() * 100).round(1)
    else:
            ...
            with cols_f[i + 1]:
                if jug_banda.empty:
                    ...
                    for _, row in jug_banda.iterrows():
                        if col_metrica == "Max Vel (% Max)":
                            valor_chico = f'{row["Velocidad_kmh"]:.1f}'.replace(".", ",") + " km/h"
                        elif col_metrica in ["Max Acceleration", "Max Deceleration"]:
                            valor_chico = f'{row["Valor_Sesion"]:.1f}'.replace(".", ",") + " m/s²"
                        else:
                            valor_chico = f'{row["Valor_Sesion"]:.1f}'.replace(".", ",")
                        items_html += (
                            ...
                            f'<div style="font-size:10px; color:#5a7a90;">{valor_chico}</div>'
                            f'</div>'
...
_partidos_jug_g = df_raw[df_raw["MD"] == "MD"].groupby("Player Name")["Fecha"].nunique()
_jug_validos_g = _partidos_jug_g[_partidos_jug_g >= 6].index
todos_jug_g = sorted(df_raw[df_raw["Player Name"].isin(_jug_validos_g)]["Player Name"].dropna().unique().tolist())
