...
grupos_sel_ca = [g for g in GRUPOS_PUESTO.keys() if st.session_state.get(f"pue_grupo_{g}", False)]
pue_sel = [p for g in grupos_sel_ca for p in GRUPOS_PUESTO[g]]
...
with col_lbl3:
    st.markdown('<div style="font-size:11px; font-weight:700; color:white; text-transform:uppercase; letter-spacing:1px;"></div>', unsafe_allow_html=True)
...
def promediar_sesion(df_input, aplicar_filtro_minutos=True):
    """Promedia métricas por sesión filtrando jugadores con >30 minutos."""
    if aplicar_filtro_minutos:
        ...
        df_input = df_input.merge(jugadores_validos, on=["SemanaInicio", "Etiqueta", "EtqCorta", "Fecha", "Player Name"])

    base = (
        df_input.groupby(["SemanaInicio", "Etiqueta", "EtqCorta", "Fecha"])[COLS]
        .mean()
        .reset_index()
        .sort_values("Fecha")
...
detalle_base = promediar_sesion(df, aplicar_filtro_minutos=(len(jug_sel) != 1))
